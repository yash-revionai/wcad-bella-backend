...
import { sendDeveloperAlert } from "../services/notifications.js";
...
callbackRequestRouter.post("/", validateApiKey, async (req, res) => {
  ...
  try {
    const parsed = callbackRequestSchema.parse(req.body);
    ...
    const alertSent = await sendDeveloperAlert({
      subject: `Bella callback request: ${categoryLabel(parsed.category)}`,
      text: [
        "Bella collected a callback request.",
        "",
        `Category: ${categoryLabel(parsed.category)} (${parsed.category})`,
